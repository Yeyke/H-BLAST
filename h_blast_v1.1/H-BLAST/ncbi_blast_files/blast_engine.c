...
static Boolean sufficient_memory[2]= {FALSE,FALSE};
...
/* ********** FINISH ************* */
{
    ...
    while (TRUE)
    {
        ...
        if(gpu_options->use_gpu > 0 )
        {
            hits=h_Hits[ (subject_oid)*4 ];
            successful_extensions = h_Hits[ (subject_oid)*4 + 1];
            hits_extended = h_Hits[ (subject_oid)*4 + 2];
            GPUBlastInitHitList_base = h_Hits[ (subject_oid)*4 + 3];
        }
        ...
        if (aux_struct->WordFinder)
        {
            ...
            /// the condition (GPUBlastInitHitList_base == -1) means the subject needs to be re-hit
            if( (!gpu_options->use_gpu > 0 ) || (GPUBlastInitHitList_base == -1) )
            {
...
//            for every successful extension save the results in the init_hitlist
                ...
                for( i = 0; i < successful_extensions; ++i)
                    BlastSaveInitHsp(init_hitlist,
                                     ...
                                     h_GPUBlastInitHitList[GPUBlastInitHitList_base + i].hsp_q
                                     +h_GPUBlastInitHitList[GPUBlastInitHitList_base + i].offset_from_first_position,
                                     h_GPUBlastInitHitList[GPUBlastInitHitList_base + i].hsp_s
                                     +h_GPUBlastInitHitList[GPUBlastInitHitList_base + i].offset_from_first_position,
                                        ...
                                        TInterruptFnPtr interrupt_search, SBlastProgress* progress_info)
{
    ...
    if( (gpu_options->use_gpu > 0) && available_gpu )
    {
        /// ++++++++++++++++++++++  Thread & CPU binding ++++++++++++

        int processor_count=get_processor_no();

        int core_count_per_processor=get_core_count_per_processor();

        int previous_cpu_thread_count = (((gpu_options->use_gpu+processor_count-1)/processor_count)-1)*thread_counter2
                                        + (gpu_options->use_gpu%processor_count)*core_count_per_processor;

        unsigned long mask = 1<<(threadId+previous_cpu_thread_count);
    ...
    {
        /// ++++++++++++++++++++++  Thread & CPU binding ++++++++++++
        int previous_cpu_thread_count = 0 - gpu_options->use_gpu;
        //printf("pre_cpu is %d\n", previous_cpu_thread_count);
    ...
    //if( !available_gpu )
    //{
    /// Clean the thread_counter;
    if(0==threadId) thread_counter2 = 0;
//        pthread_mutex_lock(&thread_counter2_mutex);
//        thread_counter2--;
//        pthread_mutex_unlock(&thread_counter2_mutex);
    //}
